...
using AbpSolution1.Blazor.Components;
using Volo.Abp;
...
public class AbpSolution1BlazorModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
        {
            options.AddAssemblyResource(
                ...
                typeof(AbpSolution1BlazorModule).Assembly
            );
        });
    ...
    private void ConfigureBundles()
    {
        ...
        Configure<AbpBundlingOptions>(options =>
        {
            ...
            globalStyles.AddContributors(typeof(AbpSolution1StyleBundleContributor));

            var globalScripts = options.ScriptBundles.Get(BlazorWebAssemblyStandardBundles.Scripts.Global);
            globalScripts.AddContributors(typeof(AbpSolution1ScriptBundleContributor));
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<AbpSolution1DomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}AbpSolution1.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpSolution1DomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}AbpSolution1.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpSolution1ApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}AbpSolution1.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpSolution1ApplicationModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}AbpSolution1.Application"));
                options.FileSets.ReplaceEmbeddedByPhysical<AbpSolution1BlazorModule>(hostingEnvironment.ContentRootPath);
            });
        }
    }
    ...
    private void ConfigureRouter(ServiceConfigurationContext context)
    {
        ...
        {
            options.AppAssembly = typeof(AbpSolution1BlazorModule).Assembly;
            options.AdditionalAssemblies.Add(typeof(AbpSolution1BlazorClientModule).Assembly);
        });
    }
